...
def tuple_sum(A, B):
    ...
    [(4, 6), (40, 60)]
    '''
    zipped_items = [list(zip(list(a), list(B[i]))) for i, a in enumerate(A)]
    results = []

    for i in zipped_items:
        results.append((sum(i[0]), sum(i[1])))
...
comprehension_without_row = [[x + i for i in range(20)] for x in range(15)]
