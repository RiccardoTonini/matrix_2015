...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    unmatched_items = set(A.f.items()) ^ set(B.f.items())
...
def add(A, B):
    ...
    for (key, val) in B.f.items():
        f_sum.update({key: val + f_sum.get(key, 0)})

    return Mat(A.D, f_sum)
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {key: x * val for (key, val) in M.f.items()})
...
def transpose(M):
    ...
    True
    """
    return Mat((M.D[1], M.D[0]), {(y, x): val for ((x, y), val) in M.f.items()})
