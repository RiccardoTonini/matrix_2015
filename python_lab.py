...
X2 = { 32, 20, 12.5}  # 32 		20   	12.5
Y2 = { 8, 5, 3.125}  #  8 5 3.125
...
list_average = sum([n for n in list_of_numbers]) // len(list_of_numbers)
...
LofL_sum = sum([sum([x for x in inner_list]) for inner_list in  LofL])


## 13: (Task 13) Three-element tuples summing to zero
"""
￼Task 13: Suppose S is a set of integers,
Write a triple comprehension
whose value is a list of all three-element tuples (i, j, k)
such that i, j, k are elements of S whose sum is zero.
"""
S = {-4, -2, 1, 2, 5, 0}
...
odd_num_list_range = {x for x in range(1, 100) if x % 2}
...
list_sum_zip = [a[0] + a[1] for a in zip(A, B)]
...
representation_dict = {((base**2)*hundred + base*ten + unit):(hundred, ten, unit) for hundred in digits for ten in digits for unit in digits}
...
listdict2dict = {names[k]:v for (k,v) in id2salary.items() if names[k]}
