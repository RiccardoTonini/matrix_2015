...
def equal(u, v):
    ...
    """
    assert u.D == v.D
    unmatched_items = set(u.f.items()) ^ set(v.f.items())
...
def add(u,v):
    ...
    for (key, val) in v.f.items():
        sum_val = val + f_sum.get(key, 0)
        f_sum.update({key: sum_val})

    return Vec(u.D | v.D, f_sum)
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([val * getitem(v, key) for (key, val) in u.f.items() if getitem(v, key)])
