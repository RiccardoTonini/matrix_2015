...
indep_vec_3 = Vec({0,1,2}, {0:1, 1:0, 2:0})
...
def is_superfluous(S, v):
    ...
    if len(S) <= 1:
        return False
    A = coldict2mat([vec for vec in S if not v == vec])
    b = solve(A, v)
    residue = v - A * b
...
def exchange(S, A, z):
    ...
    '''
    S_minus_A = S - A
    for coeff, vec in zip(list(vec2rep(S, z)), list(S)):
        if coeff != 0 and vec in S_minus_A:
            return vec
    return None
